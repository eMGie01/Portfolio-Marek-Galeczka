...
// Function for reading from the buffer
...
// Function for reading from the buffer without deleting data
// Function for reading from the buffer
size_t cb_buffer_peek (circ_buffer_t *cb, uint8_t *data, size_t len) {
	if (cb_is_empty(cb) || len == 0) return 0;
